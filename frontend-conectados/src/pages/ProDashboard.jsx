...
const ProDashboard = () => {
  ...
  const loadProData = async () => {
    if (user && Array.isArray(user.roles) && user.roles.includes("PRESTADOR")) {
      try {
        ...
        const citasResponse = await fetch(
          `http://localhost:8080/api/citas/prestador/${user.id}`
        );
  ...
  if (!Array.isArray(user.roles) || !user.roles.includes("PRESTADOR")) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Esta acción no se puede deshacer. ¿Estás seguro que deseas
              eliminar el servicio <strong>{serviceToDelete?.nombre}</strong>?
            </p>
